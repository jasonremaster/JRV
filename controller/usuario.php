    ...
    switch($_GET["op"]){
        case "guardaryeditar":
            if(empty($_POST["usu_id"])){
                ...
                //verificamos cedula
                $cedula = $usuario->get_usuario_telf($_POST["usu_telf"]);
                if(count($cedula)==0){
                    
                    $ced =true;
                }else{
                    $messages[] = "Cédula ya registrada en el sistema  - "; 
                    $ced =false;
                }
                //verificamos correo
                $correo = $usuario->get_usuario_correo($_POST["usu_correo"]);
                if(count($correo)==0){
                    
                    $cor =true;
                }
                else{
                    $messages[] = " Correo ya registrado en el sistema ";
                    $cor =false;
                }

                if($ced ==true and $cor==true){
                ...
                echo json_encode(array("success" => true, "messages" => $messages, "usu_telf"=>$_POST["usu_telf"]));
                }
                else{
